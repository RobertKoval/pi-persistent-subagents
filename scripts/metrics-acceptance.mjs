...
try{
  ...
  const parentMessages=await Promise.all(clients.map(async c=>(await c.request({type:'get_messages'})).data.messages.filter(m=>m.role==='assistant')));
  ...
  console.log(JSON.stringify({passed:true,parents:2,workerCalls:4,samePid:true,usageMatchesSessionLogs:true,peakConcurrency:report.concurrency.peak,cacheRead:report.calls.reduce((n,c)=>n+(c.usage?.cacheRead??0),0),observedStreamCalls:report.calls.filter(c=>c.observed_tps!==null).length,workersGone:true}));
}finally{
